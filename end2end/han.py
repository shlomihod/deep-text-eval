...
def han_model():
    ...
    embeddings = {}
    with open('glove_directory/glove.6B.100d.txt', encoding='utf-8') as file:
        for line in file:
            values = line.split()
            word = values[0]
    ...
    model = HAN(MAX_WORDS_PER_SENT, MAX_SENT, 5, embedding_matrix,word_encoding_dim=100, sentence_encoding_dim=100)
    model.summary()
    model.compile(optimizer='adam', loss='categorical_crossentropy',metrics=['acc'])
    history = model.fit(X_train, y_train, epochs=10,validation_data=(X_test, y_test))

    plot_accuracy_and_loss_curves(history,"categorical_crossentropy","han")
    model.save("models/han.h5")
    ...
    train_accuracy = model.evaluate(X_test, y_test, verbose=0)
    # Making predictions
